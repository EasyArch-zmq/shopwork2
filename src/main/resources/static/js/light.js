	...
	$(document).ready(() => {
		...
		console.log(username)
			$.ajax({
				...
				success: (res) => {
					console.log(res)
				   document.getElementsByTagName('h2')[0].innerHTML = res.userAddress;
					var arry=[];
					arry=res.list;
				   // getaddress(res,"light-address");
					...
					if(arry[i].number<=res.color.green)
						img_light.src = "../../assets/img/green.png";
					else
					   if(arry[i].number<=res.color.red)
						   img_light.src = "../../assets/img/yellow.png";
					   else
						   img_light.src = "../../assets/img/red.png";
